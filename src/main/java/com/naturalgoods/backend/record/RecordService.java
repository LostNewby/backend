...
public class RecordService {
    ...
    public List<ProductCardsDto> filter(String name, List<Long> categoryId, List<Long> productId, List<Long> productTypeId, SortingType sortingType, Long minPrice, Long maxPrice, String region){
        ...
        builder.append("select r.id, r.region, r.price from records r left join product_type pt on pt.id = r.product_type_id left join product p on p.id = pt.product_id left join category c on c.id = p.category_id where r.region = :region and r.price >= :minPrice  ");
        ...
        if(Objects.nonNull(name) && !name.equals("")){
            builder.append("and pt.name_kz like :name or pt.name_ru like :name or pt.name_en like :name ");
        }
